...
TEST_CASE("Spherical Quadratures", "[sph-quad]") {
  ...
  SECTION("Batching") {
    ...
    IntegratorXX::SphericalMicroBatcher batcher = make_batcher(max_batch_sz, s);
    ...
    batcher.quadrature().recenter(
        {0., 0., 0.});  // just check if this compiles....
  }
}
...
TEST_CASE("Pruned Spherical Quadratures", "[sph-quad]") {
  SECTION("Consistency With Spherical Quadrature") {
    ...
    auto comparator_s = [&](auto i, auto j) { return comparator_pt(s, i, j); };
  ...
  SECTION("Validity") {
    size_t nrad = 12;
    IntegratorXX::MuraKnowles<double, double> r(nrad);
    IntegratorXX::LebedevLaikov<double> q_6(6), q_14(14), q_302(302),
        q_770(770);

    IntegratorXX::RadialGridPartition rgp(r, 0ul, q_6, 4ul, q_302, 6ul, q_14,
                                          9ul, q_770);
    IntegratorXX::PrunedSphericalQuadrature ps(r, rgp);
    ...
    for(const auto& [r_range, q] : rgp) {
      ...
      for(auto j = 0; j < q.npts(); ++j)
        for(auto i = r1; i < r2; ++i) {
          ...
          CHECK(ps.weights()[npts] ==
                Catch::Approx(4 * M_PI * _r * _r * _rw * _aw));
          npts++;
        }
    }
    CHECK(npts == ps.npts());
  ...
  SECTION("With Batcher") {
    ...
    IntegratorXX::SphericalMicroBatcher batcher_s = make_batcher(1, s);
    IntegratorXX::SphericalMicroBatcher batcher_ps = make_batcher(1, ps);
