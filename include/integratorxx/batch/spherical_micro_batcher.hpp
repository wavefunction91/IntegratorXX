...
template <typename T>
bool point_in_box(const cartesian_pt_t<T>& lo, const cartesian_pt_t<T>& up,
                  const cartesian_pt_t<T>& pt) {
  if(pt[0] > up[0] or pt[0] < lo[0])
    return false;
  else if(pt[1] > up[1] or pt[1] < lo[1])
    return false;
  else if(pt[2] > up[2] or pt[2] < lo[2])
    return false;
  else
    return true;
}
...
inline constexpr bool comp_coordinate(const cartesian_pt_t<T>& a,
...
auto get_box_bounds(PWIterator pw_batch_begin, PWIterator pw_batch_end) {
  ...
  return std::make_tuple(cartesian_pt_t<cart_type>{min_x, min_y, min_z},
                         cartesian_pt_t<cart_type>{max_x, max_y, max_z});
}

template <typename PIterator>
auto get_box_bounds_points(PIterator p_batch_begin, PIterator p_batch_end) {
...
template <typename T>
auto partition_box(size_t npart, const cartesian_pt_t<T>& bbox_lo,
                   const cartesian_pt_t<T>& bbox_up,
                   pw_iterator<T> pw_batch_begin, pw_iterator<T> pw_batch_end) {
  ...
  for(auto i = 0ul; i < npart; ++i)
    for(auto j = 0ul; j < npart; ++j)
      for(auto k = 0ul; k < npart; ++k) {
        std::array<T, 3> box_lo = {x_part[i], y_part[j], z_part[k]};

        std::array<T, 3> box_up = {x_part[i + 1], y_part[j + 1], z_part[k + 1]};

        auto point_in_this_box = [&](const auto& pw) {
          return detail::point_in_box(box_lo, box_up, pw.first);
        };

        partition_its.emplace_back(std::partition(
            partition_its.back(), pw_batch_end, point_in_this_box));
      }
...
template <typename PWIterator>
auto partition_box(size_t npart, PWIterator pw_batch_begin,
...
class SphericalMicroBatcher {
  ...
  struct iterator {
    ...
    auto range() {
      ...
      return std::make_tuple(npts, point_begin + idx, point_begin + idx + npts,
                             weight_begin + idx, weight_begin + idx + npts);
    }
    ...
    value_type operator*() {
      ...
      return std::make_tuple(box_lo, box_up, std::vector<point_type>(pb, pe),
 ...
 public:
  ...
  SphericalMicroBatcher(size_t batch_size, const Quad& quad)
      : SphericalMicroBatcher(batch_size, std::make_shared<Quad>(quad)) {}

  template <typename Quad, typename = enable_if_spherical_quad_t<Quad>>
  SphericalMicroBatcher(size_t batch_size, const Quad& quad,
                        index_container idx)
      : SphericalMicroBatcher(batch_size, std::make_shared<Quad>(quad), idx) {}

  template <typename Quad, typename = enable_if_spherical_quad_t<Quad>>
  SphericalMicroBatcher(size_t batch_size, Quad&& quad)
      : SphericalMicroBatcher(batch_size,
                              std::make_shared<Quad>(std::move(quad))) {}

  template <typename Quad, typename = enable_if_spherical_quad_t<Quad>>
  SphericalMicroBatcher(size_t batch_size, Quad&& quad, index_container idx)
      : SphericalMicroBatcher(batch_size,
                              std::make_shared<Quad>(std::move(quad)), idx) {}
  ...
  typename iterator::value_type at(size_t i) {
    if(i >= nbatches()) throw std::runtime_error("Index out of bounds");
  ...
  inline void recenter(point_type new_center) { quad_->recenter(new_center); }
};

template <typename PointContainer, typename WeightContainer>
std::vector<size_t>
SphericalMicroBatcher<PointContainer, WeightContainer>::generate_batches() {
  ...
  while(std::any_of(part_counts.begin(), part_counts.end(), check_max_batch)) {
#if INTEGRATORXX_VERBOSITY_LEVEL > 0
    std::cout << "REFINE ITER = " << std::setw(4) << refine_iter
              << " NBATCH = " << std::setw(6) << nparts << " MAX BATCH  = "
              << *std::max_element(part_counts.begin(), part_counts.end())
              << std::endl;
#endif
    refine_iter++;
    ...
    for(auto ip = 0ul; ip < nparts; ++ip)
      if(check_max_batch(part_counts[ip])) {
        ...
        auto batch_part_its = detail::partition_box(
            npart_batch, bbox_lo, bbox_up, batch_st, batch_en);
    ...
    std::set_union(partition_its.begin(), partition_its.end(),
...
#if INTEGRATORXX_VERBOSITY_LEVEL > 0
  std::cout << std::endl;
  std::cout << "FINAL NBATCH = " << nparts << "     FINAL MAX BATCH = "
            << *std::max_element(part_counts.begin(), part_counts.end())
            << std::endl;
#endif
...
SphericalMicroBatcher<typename QuadType::point_container,
...
make_batcher(size_t batch_size, const QuadType& quad) {
  ...
  return SphericalMicroBatcher<point_container, weight_container>(batch_size,
                                                                  quad);
}
