...
class GenerateQuadrature<Lebedev<PointType, wght_t, ContiguousContainer> > {
  using point_container =
      typename Lebedev<PointType, wght_t, ContiguousContainer>::point_container;
  using weight_container =
      typename Lebedev<PointType, wght_t,
                       ContiguousContainer>::weight_container;

  /**
   *  \brief Generate the Lebedev-Laikov quadrature rule of a specific order
   * (impl)
   *
