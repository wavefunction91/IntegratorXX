...
class QuadratureBatch {
  ...
  class iterator {
   ...
   public:
    using value_type = std::tuple<point_container, weight_container>;
    ...
    iterator(const QuadratureType& q, const size_t indx, const size_t bsz = 1)
        : quad(&q), cur_indx(indx), batch_sz(bsz) {}

    iterator& operator++() {
      cur_indx += std::min(batch_sz, quad->nPts() - cur_indx);
      return *this;
    }
    ...
    bool operator==(iterator other) {
      return cur_indx == other.cur_indx and quad == other.quad;
    }
    bool operator!=(iterator other) { return not(*this == other); }

    auto range() {
      size_t ncpy = std::min(batch_sz, quad->nPts() - cur_indx);
    ...
    auto operator*() {
      ...
      std::copy(quad->points().begin() + cur_indx,
                quad->points().begin() + cur_indx + ncpy, pts.begin());

      std::copy(quad->weights().begin() + cur_indx,
                quad->weights().begin() + cur_indx + ncpy, wgt.begin());
 ...
 public:
  ...
  QuadratureBatch(const QuadratureType& q, const size_t bsz = 1)
      : batch_sz(bsz), quad(q){};
  ...
  inline size_t max_batch_size() const { return batch_sz; }
};
...
struct spherical_from_radial_cart_combine_op {
  template <typename T, typename U, typename V>
  static std::enable_if_t<std::is_same_v<cartesian_pt_t<U>, T> and
 ...
 *  fly.
 */
template <typename CombinedType, typename QuadratureType1,
          typename QuadratureType2,
...
class QuadratureBatch2D_t {
 protected:
  ...
  class iterator {
   protected:
    using batch_iterator_1 = typename QuadratureBatch2D_t::batch_iterator_1;
    using batch_iterator_2 = typename QuadratureBatch2D_t::batch_iterator_2;

    using weight_type =
        decltype(std::declval<typename QuadratureType1::weight_type>() *
   ...
   public:
    using value_type = std::tuple<CombinedType, weight_container>;
    using pointer = const value_type*;
    using reference = value_type&&;
    using difference_type = size_t;
    using iterator_catagory = std::forward_iterator_tag;
    ...
    iterator(const decltype(q_batch_1)& q1, const decltype(q_batch_2)& q2,
    ...
    bool operator==(iterator other) {
      return it_1 == other.it_1 and it_2 == other.it_2;
    }
    bool operator!=(iterator other) { return not(*this == other); }
 ...
 public:
  QuadratureBatch2D_t(const QuadratureType1& q1, const QuadratureType2& q2,
                      const size_t bsz1, const size_t bsz2)
      : q_batch_1(q1, bsz1),
        q_batch_2(q2, bsz2),
        batch_sz_1(bsz1),
        batch_sz_2(bsz2) {}
  ...
  iterator end() {
    return iterator(q_batch_1, q_batch_2, 0, q_batch_2.n_batches());
  }
  ...
  inline size_t max_batch_size() const {
    return q_batch_1.max_batch_size() * q_batch_2.max_batch_size();
  }
};
 ...
 *  fly.
 */
template <typename CombinedType, typename RadialQuadrature,
          typename AngularQuadrature,
          typename CombineOp = detail::spherical_from_radial_cart_combine_op>
class SphericalBatch_t
    : public QuadratureBatch2D_t<CombinedType, RadialQuadrature,
                                 AngularQuadrature, CombineOp> {
  using batch_base = QuadratureBatch2D_t<CombinedType, RadialQuadrature,
                                         AngularQuadrature, CombineOp>;
  ...
  class iterator : public batch_base::iterator {
   ...
   public:
    ...
    iterator(const decltype(center) cen, const decltype(scale) scl,
             Args&&... args)
        : center(cen),
          scale(scl),
          batch_base::iterator(std::forward<Args>(args)...) {}
    ...
    auto operator*() {
      ...
      for(auto [pi, wi] = mod_loop_init; pi != pts.end(); ++pi, ++wi) {
        *wi *= 4 * M_PI * (*r_it) * (*r_it) * scale * scale * scale;
        ++r_it;
 ...
 public:
  template <typename... Args>
  SphericalBatch_t(const decltype(center) cen, const decltype(scale) scl,
                   Args&&... args)
      : center(cen), scale(scl), batch_base(std::forward<Args>(args)...) {}

  iterator begin() {
    return iterator(center, scale, this->q_batch_1, this->q_batch_2, 0, 0);
  }
  iterator end() {
    return iterator(center, scale, this->q_batch_1, this->q_batch_2, 0,
                    this->q_batch_2.n_batches());
  }
};
...
class SphericalMicroBatch_t {
  ...
  std::vector<std::tuple<point_type,               // low bound
                         point_type,               // up bound
  ...
  void generate_micro_batches() {
    ...
    const auto micro_offset =
        ((n_macro_subdivide - n_micro_subdivide) / 2) * macro_step;
    ...
    std::vector<std::tuple<point_type, double>> sphere_zip(
        sphere_points.size());

    for(auto i = 0; i < sphere_points.size(); ++i)
      sphere_zip[i] = {sphere_points[i], sphere_weights[i]};

    const point_type box_lo_bound = {center[0] - 0.5 * box_dim,
                                     center[1] - 0.5 * box_dim,
                                     center[2] - 0.5 * box_dim};

    const point_type box_up_bound = {center[0] + 0.5 * box_dim,
                                     center[1] + 0.5 * box_dim,
                                     center[2] + 0.5 * box_dim};

    const point_type micro_box_lo_bound = {box_lo_bound[0] + micro_offset,
                                           box_lo_bound[1] + micro_offset,
                                           box_lo_bound[2] + micro_offset};

    const point_type micro_box_up_bound = {box_up_bound[0] - micro_offset,
    ...
    /*
          ...
          std::cout << "BOX BOUNDS:  " << box_lo_bound << " -> " << box_up_bound
       << std::endl; std::cout << "MACRO STEP " << macro_step << std::endl;
          std::cout << "MICRO STEP " << micro_step << std::endl;
          std::cout << "MICRO OFFSET " << micro_offset << std::endl;
          std::cout << "MICRO BOX BOUNDS:  " << micro_box_lo_bound << " -> " <<
       micro_box_up_bound << std::endl;
    */

    constexpr auto point_in_box = [](const auto& pt, const auto& box_lo,
                                     const auto& box_hi) {
    ...
    auto point_in_micro_box = [&](const auto& pt) {
      return point_in_box(pt, micro_box_lo_bound, micro_box_up_bound);
    };

    constexpr auto segregate_boxes = [](const auto& lo_bnd, const auto& up_bnd,
                                        const auto nseg) {
    ...
    const auto center_box =
        std::find_if(big_boxes.begin(), big_boxes.end(), [&](const auto& bx) {
          return point_in_box(point_type({0., 0., 0.}), std::get<0>(bx),
                              std::get<1>(bx));
        });
    ...
    for(auto bx_it = big_boxes.begin(); bx_it != big_boxes.end(); bx_it++) {
      ...
      for(const auto& b : boxes)
        micro_batches.push_back({std::get<0>(b), std::get<1>(b),
    ...
    /*
          ...
          for( auto iPt = 0; iPt < sphere_points.size(); ++iPt )
          if( (std::find(indx_keep.begin(), indx_keep.end(), iPt) ==
       indx_keep.end()) and (point_in_box( sphere_points[iPt], box_lo_bound,
       box_up_bound ) or (sphere_points[iPt][0] == box_up_bound[0]) or
    ...
    */
    ...
    /*
       ...
       std::get<2>(y).size(); } ) << std::endl;

    */
  }

 public:
  SphericalMicroBatch_t(const RadialQuadrature& r, const AngularQuadrature& l,
                        const point_type cen, const radial_point_type scl,
                        const radial_point_type b_dim, const int64_t n_macro,
...
QuadratureBatch2D(const QuadratureType1& q1, const QuadratureType2& q2,
                  const size_t bsz1 = 1, const size_t bsz2 = 1) {
  return QuadratureBatch2D_t<CombinedType, QuadratureType1, QuadratureType2,
                             CombineOp>(q1, q2, bsz1, bsz2);
}

template <typename RadialQuadrature,
          typename point_type = typename RadialQuadrature::point_type>
SphericalBatch_t<cartesian_pt_t<point_type>, RadialQuadrature,
                 Lebedev<point_type>>
SphericalBatch(const RadialQuadrature& r, const Lebedev<point_type>& l,
               const cartesian_pt_t<point_type> cen = {0., 0., 0.},
               const point_type scale = 1., const size_t bsz1 = 1,
               const size_t bsz2 = 1) {
  return SphericalBatch_t<cartesian_pt_t<point_type>, RadialQuadrature,
                          Lebedev<point_type>>(cen, scale, r, l, bsz1, bsz2);
}

template <typename RadialQuadrature,
          typename point_type = typename RadialQuadrature::point_type>
SphericalMicroBatch_t<cartesian_pt_t<point_type>, RadialQuadrature,
                      Lebedev<point_type>>
SphericalMicroBatch(const RadialQuadrature& r, const Lebedev<point_type>& l,
                    const cartesian_pt_t<point_type> cen = {0., 0., 0.},
                    const point_type scale = 1.) {
  return SphericalMicroBatch_t<cartesian_pt_t<point_type>, RadialQuadrature,
                               Lebedev<point_type>>(r, l, cen, scale, 66., 6,
                                                    2);
}
};  // namespace IntegratorXX::deprecated
