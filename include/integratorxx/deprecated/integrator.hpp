...
/*
  ...
  struct is_increment_function_sig<
    Func,
    RetType,
    std::void_t<decltype(std::declval<Func>(std::declval<RetType&>(),
  std::declval<Args>()...))>, Args... > : std::true_type { };
*/
...
class Integrator1D {
 ...
 public:
  ...
  Integrator1D(const QuadType& q, const size_t bsz = 1) : qb(q, bsz) {}

  // TODO: SFINAE on function signature
  template <typename RetType, typename Func, typename... Args>
  /*
      std::enable_if_t<
        is_increment_function_sig< Func, RetType, const typename
     QuadType::point_container&, const typename QuadType::weight_container&,
          Args... >::value,
        RetType
      >
  */
  RetType integrate(const Func& func, RetType&& init, Args&&... args) {
